  ...
  <div class="container mx-auto p-4 relative">
    <div class="mb-4 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <button
v-if="video?.compiledTracking" class="btn btn-ghost" :class="{ 'btn-active': showCalibrated }"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 bg-base-200 p-4 rounded-lg">
        <div class="relative">
          <video
ref="videoRef" :src="`${baseURL}/uploads/videos/${video?.filename}`" class="w-full rounded-lg"
          ...
          <canvas
ref="trackingCanvasRef" class="absolute top-0 left-0 w-full h-full" @mousedown="handleMouseDown"
            @mousemove="handleMouseMove" @mouseup="handleMouseUp" @contextmenu.prevent="handleRightClick"></canvas>
          <video
ref="depthVideoRef" :src="`${baseURL}/uploads/videos/${video?.depthFilename}`"
            class="hidden" crossorigin="anonymous" @loadedmetadata="handleDepthVideoLoaded"></video>
        </div>

        <div
v-if="depthCheckActive && lastDepthCheck"
        ...
        <div class="bg-base-200 p-4 rounded-lg h-3/4">
          ...
          <div class="space-y-4 max-h-[calc(100%-4rem)] overflow-y-auto">
            <div v-for="obj in objects" :key="obj.id" class="flex items-center justify-between p-2 rounded bg-base-100">
              ...
              <div class="flex gap-2">
                <button
class="btn btn-sm btn-ghost" :class="{ 'btn-active': selectedObject?.id === obj.id }"
        ...
        <div class="bg-base-200 p-4 rounded-lg h-1/4">
          <div class="flex justify-between items-center mb-4">
            ...
            <button
class="btn btn-sm btn-secondary" :class="{ 'btn-active': depthCheckActive }"
              @click="depthCheckActive = !depthCheckActive">
              Check Depth {{ lastDepthCheck ? `(${lastDepthCheck.depth.toFixed(3)})` : '' }}
            </button>
            <button
class="btn btn-sm btn-primary" :class="{ 'btn-active': trackingMode === 'calibration' }"
          ...
          <div class="space-y-2 max-h-[calc(100%-4rem)] overflow-y-auto">
            <div
v-for="(point, index) in calibrationPoints" :key="index"
      ...
      <div class="modal-box">
        ...
        <form @submit.prevent="addCalibrationPoint">
          <div class="form-control">
            ...
            <input
v-model.number="newCalibrationPoint.distance" type="number" step="0.1" min="0.1"
